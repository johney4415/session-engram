...
1. If the argument looks like a session id (hex, at least 4 characters), run
   `session-engram resume <id>` — a prefix is enough. Otherwise find it first
   with `session-engram list --plain --search "$ARGUMENTS" --limit 10`, and if
   several sessions could be it, show them and ask which one.
2. Give the user the printed `cd … && claude --resume …` line to run
   themselves, in a copyable block. Offer `session-engram resume <id> --copy` to
